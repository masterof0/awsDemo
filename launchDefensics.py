...
#Create key pair
if conn.get_key_pair(args.name):
  if not os.path.exists(awsDir + args.name + '.pem'):
    sys.exit("This name is already taken. Please launch again with new name")
else:
    key = conn.create_key_pair(args.name)
    key.save(awsDir)
...
print "Please note it may take up to 30 minutes for the images to launch and be fully available"
...
for index, i in enumerate(instances):
  commonName = args.name + ':' + str(index) + '_' + reservation.id
  awsDB.execute("insert into instances values (?,?,?,?,null,?)", (i.id, reservation.id, commonName, i.ip_address, str(i._state)))
  ...
  with open('.aws.log', 'a') as log:
    ...
    log.write('reservation id: ' + reservation.id + '\n')
    log.write('base name: ' + args.name + '\n')
    log.write('instances: ' + str(instances) +'\n')
    log.write('Please note it may take up to 30 minutes for the images to launch and be fully available')
    log.write('\n\n')
  log.closed
